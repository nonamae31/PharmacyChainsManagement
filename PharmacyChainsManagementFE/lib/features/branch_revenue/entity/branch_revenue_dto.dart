...
class CategoryRevenueDto extends Equatable {
  ...
  const CategoryRevenueDto({required this.category, required this.revenue, required this.contributionPercent});

  factory CategoryRevenueDto.fromJson(Map<String, dynamic> json) => CategoryRevenueDto(
        category: json['category']?.toString() ?? '',
        revenue: (json['revenue'] as num?)?.toDouble() ?? 0,
        contributionPercent: (json['contributionPercent'] as num?)?.toDouble() ?? 0,
      );

  Map<String, dynamic> toJson() => {'category': category, 'revenue': revenue, 'contributionPercent': contributionPercent};
...
class TimeBlockPerformanceDto extends Equatable {
  ...
  factory TimeBlockPerformanceDto.fromJson(Map<String, dynamic> json) => TimeBlockPerformanceDto(
  ...
  @override
  List<Object?> get props => [timeBlock, transactions, revenue, averageOrder, status];
}
...
class BranchRevenueDto extends Equatable {
  ...
  factory BranchRevenueDto.fromJson(Map<String, dynamic> json) => BranchRevenueDto(
        ...
        revenueTrend: (json['revenueTrend'] as List<dynamic>? ?? const [])
            .map((item) => RevenuePointDto.fromJson(item as Map<String, dynamic>)).toList(growable: false),
        categoryRevenue: (json['categoryRevenue'] as List<dynamic>? ?? const [])
            .map((item) => CategoryRevenueDto.fromJson(item as Map<String, dynamic>)).toList(growable: false),
        performanceByTime: (json['performanceByTime'] as List<dynamic>? ?? const [])
            .map((item) => TimeBlockPerformanceDto.fromJson(item as Map<String, dynamic>)).toList(growable: false),
      );
  ...
  Map<String, dynamic> toJson() => {
        ...
        'performanceByTime': performanceByTime.map((item) => item.toJson()).toList(),
      };

  @override
  List<Object?> get props => [branchId, fromDate, toDate, totalRevenue, averageTicket, transactions, grossMarginPercent, revenueTrend, categoryRevenue, performanceByTime];
}
