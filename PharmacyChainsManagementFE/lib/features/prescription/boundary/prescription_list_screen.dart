...
class PrescriptionListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BlocProvider(
    create: (_) => sl<PrescriptionBloc>()..add(PrescriptionListRequested()),
      ...
      builder: (context, state) {
        ...
        if (state is PrescriptionListLoadSuccess) {
          if (state.prescriptions.isEmpty)
            return const Center(child: Text('Chua co don thuoc.'));
          return RefreshIndicator(
            onRefresh: () async => context.read<PrescriptionBloc>().add(
              PrescriptionListRequested(),
            ),
            child: ListView.separated(
              padding: const EdgeInsets.all(16),
              itemCount: state.prescriptions.length,
              separatorBuilder: (_, __) => const SizedBox(height: 8),
              itemBuilder: (context, index) => _PrescriptionListTile(
                prescription: state.prescriptions[index],
                onTap: () => context.push(
                  '/staff/prescriptions/${state.prescriptions[index].prescriptionId}',
                ),
              ),
            ),
          );
...
class _PrescriptionListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    child: ListTile(
      ...
      title: Text(prescription.customerName),
      subtitle: Text(
        '${prescription.prescriptionDate} - ${prescription.itemCount} thuoc${prescription.doctorName == null ? '' : '\nBS. ${prescription.doctorName}'}',
      ),
      trailing: Text(prescription.status),
    ),
  );
}
